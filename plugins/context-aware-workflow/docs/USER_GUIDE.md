...
> **업데이트**: **Tidy First (Kent Beck) 방법론**, 자동 병렬 실행, Git Worktree 지원, Ralph Loop, Serena 동기화
...
1. [빠른 시작](#-빠른-시작)
2. [핵심 개념](#-핵심-개념)
3. [Tidy First 방법론](#-tidy-first-방법론-new) **(NEW)**
4. [명령어 상세](#-명령어-상세)
5. [에이전트](#-에이전트)
6. [스킬](#-스킬)
7. [워크플로우 예시](#-워크플로우-예시)
...
```
1. Planner가 각 Phase에 필요한 Tidy step 자동 생성
2. Builder가 Tidy step 실행 → [tidy] 커밋
3. Builder가 Build step 실행 → [feat] 커밋
...
| 플래그 | 설명 |
|--------|------|
| `--scope <path>` | 특정 파일 또는 디렉토리 분석 |
| `--preview` | 미리보기만, 변경 없음 |
| `--apply` | 분석된 변경 자동 적용 |
| `--add-step` | task_plan.md에 Tidy step 추가 |
| `--commit` | `[tidy]` 프리픽스로 커밋 |
...
### PreToolUse 훅 - Tidy First 커밋 검증

`git commit` 명령 실행 전 **Tidy First 규칙**을 자동 검증합니다.
...
- [x] **Tidy First 방법론** (v1.6.0) - Kent Beck 커밋 분리 규칙
- [x] 모든 핵심 에이전트 (9개)
- [x] 모든 핵심 스킬 (16개)
