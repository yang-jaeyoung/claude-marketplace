...
| Phase | 실행 방법 | 도구 | 비고 |
|-------|----------|------|------|
| **Phase 1** | **3개 에이전트 즉시 동시 실행** | Task ×3 | 블로킹 없음 |
| Phase 2 | 기술분석 + 리포트 생성 | Write | Phase 1 완료 후 |
...
⚠️ **파일 충돌 방지**: 각 에이전트는 **자신의 전용 파일에만** 작성. **README.md는 Phase 2에서만 생성**.
...
```
{저장경로}/{종목명}_ultra_{날짜}/
├── README.md          # 메인 리포트 (Phase 2에서만 생성)
...
- [ ] Phase 1: 에이전트 3개 Task로 **동시** 실행
- [ ] Phase 2: 기술분석 + README.md 리포트 생성
